...
if ($PSBoundParameters.ContainsKey('Analyze')) {
    ...
    #TODO: Updated to scan public/private only
    $TempPath = "{0}$DS{1}" -f $PSScriptRoot,'src'
    Invoke-ScriptAnalyzer -Path $TempPath -Recurse -ExcludeRule 'PSUseShouldProcessForStateChangingFunctions','PSUseToExportFieldsInManifest'
}
...
if ($PSBoundParameters.ContainsKey('Test')) {
    ...
    #TODO: Updated the pesterconfig part
    $pesterArgs = [PesterConfiguration]::Default
    #write-host "$PSScriptRoot\Tests\"
    $pesterArgs.Run.Path = "{0}$DS{1}" -f $PSScriptRoot,'test'
    $pesterArgs.Run.Exit = $true
    $pesterArgs.Output.Verbosity = 'Detailed'
    $Result = Invoke-Pester -Configuration $pesterArgs
...
if ($PSBoundParameters.ContainsKey('Compile')) {
    ...
    if ((Test-Path $TempPath)) {
        ...
        'using namespace System.Security.Cryptography.X509Certificates' | Add-Content $TempPath2
        'using namespace System.Security.Cryptography' | Add-Content $TempPath2
...
if ($PSBoundParameters.ContainsKey('Release')) {
    ...
    Try {
        $Splat = @{
            ...
            Tag        = 'PSModule', 'Azure Active Directory', 'JWT'
